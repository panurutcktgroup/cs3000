...
public static class SettingsCommand
{
    ...
    public static async Task TestConnectionAutomaticallyAsync(bool showProgress = true)
    {
    ...
    public static async Task RunAsync()
    {
        while (true)
        {
            ...
            var choice = AnsiConsole.Prompt(
                new SelectionPrompt<string>()
                    ...
                    .AddChoices(
                        ...
                        "Configure Traceability Data Source",
                        "Test Connection",
                        "Reset Connection Settings",
                        "Manage Link Server Profiles",
                        "Back"
                    )
            );
            ...
            switch (choice)
            {
                ...
                case "Configure Traceability Data Source":
                    ConfigureTraceabilityDataSource();
                    break;
                ...
                case "Manage Link Server Profiles":
                    // Open the link server profiles management UI/command
                    LinkServerProfilesCommand.Run();
                    break;
    ...
    private static void ShowConnectionSettings()
    {
        ...
        table.AddRow("Traceability mode", _settings.TraceabilityMode.ToString());
        table.AddRow("Linked Server", _settings.TraceabilityMode == TraceabilityConnectionMode.LinkedServer
            ? ValueOrDash(_settings.LinkedServerName)
            : "[grey]Not applicable[/]");
        table.AddRow("Traceability database", ValueOrDash(
            _settings.TraceabilityMode == TraceabilityConnectionMode.LinkedServer
                ? _settings.TraceabilityDatabase
                : _settings.Database));
        table.AddRow("Traceability object", _settings.TraceabilityMode == TraceabilityConnectionMode.LinkedServer
            ? $"{ValueOrDash(_settings.TraceabilitySchema)}.{ValueOrDash(_settings.TraceabilityTable)}"
            : "[grey]Default database table[/]");
    ...
    private static void ConfigureTraceabilityDataSource()
    {
        Console.Clear();
        TuiComponents.ShowPageHeader(
            "CONFIGURE TRACEABILITY DATA SOURCE",
            "Dashboard > Settings > Traceability Data Source",
            "Choose direct database access or a SQL Server Linked Server");

        var mode = AnsiConsole.Prompt(
            new SelectionPrompt<TraceabilityConnectionMode>()
                .Title("Traceability connection [cyan]mode[/]:")
                .AddChoices(TraceabilityConnectionMode.Direct, TraceabilityConnectionMode.LinkedServer));

        if (mode == TraceabilityConnectionMode.LinkedServer && _settings.Provider != DatabaseProvider.SqlServer)
        {
            TuiComponents.ShowError("Linked Server mode requires SQL Server as the gateway provider.");
            TuiComponents.Pause();
            return;
        }

        _settings.TraceabilityMode = mode;
        if (mode == TraceabilityConnectionMode.LinkedServer)
        {
            _settings.LinkedServerName = AskRequired("Linked Server name:");
            _settings.TraceabilityDatabase = AskRequired("Remote database name:");
            _settings.TraceabilitySchema = AskRequired("Remote schema name:", "dbo");
            _settings.TraceabilityTable = AskRequired("Remote table or view name:", "SerialNumberTraceability");
        }
        else
        {
            _settings.LinkedServerName = string.Empty;
            _settings.TraceabilityDatabase = string.Empty;
            _settings.TraceabilitySchema = "dbo";
            _settings.TraceabilityTable = "SerialNumberTraceability";
        }
        ...
        try
        {
            ...
            TuiComponents.ShowSuccess($"Traceability data source set to {mode} and saved securely.");
        }
    ...
    private static string AskRequired(string prompt, string? defaultValue = null)
    {
        while (true)
        {
            var value = (defaultValue is null
                ? AnsiConsole.Ask<string>(prompt)
                : AnsiConsole.Ask(prompt, defaultValue)).Trim();
            if (!string.IsNullOrWhiteSpace(value))
            {
                return value;
            }
