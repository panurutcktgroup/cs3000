...
public static partial class ProductControlUI
{
    ...
    internal const int DefaultConnectTimeoutSeconds = 60;
    internal const int DefaultCommandTimeoutSeconds = 120;

    // Shared helpers used by all product-control files
    ...
    internal static bool TryTcpProbe(string connectionString, int probeTimeoutSeconds, out string? errorMessage)
    {
        errorMessage = null;
        try
        {
            ...
            // Named instance -> only DNS check
            if (dataSource.Contains("\\"))
            {
                var hostPart = dataSource.Split('\\', 2)[0].Trim();
                if (string.IsNullOrEmpty(hostPart)) { errorMessage = "Invalid data source (named instance missing host)."; return false; }
                try
                {
                    var addrs = System.Net.Dns.GetHostAddresses(hostPart);
                    if (addrs == null || addrs.Length == 0) { errorMessage = $"Cannot resolve host '{hostPart}'."; return false; }
                }
                catch (Exception ex) { errorMessage = $"DNS resolution failed for '{hostPart}': {ex.Message}"; return false; }
                return true;
            }
            ...
            if (dataSource.Contains(","))
            {
                ...
                host = parts[0].Trim();
                if (!int.TryParse(parts[1].Trim(), out port)) port = 1433;
            }

            if (string.IsNullOrEmpty(host)) { errorMessage = "Invalid host in data source."; return false; }

            using var tcp = new TcpClient();
            var connectTask = tcp.ConnectAsync(host, port);
            var completed = connectTask.Wait(TimeSpan.FromSeconds(Math.Max(1, probeTimeoutSeconds)));
            if (!completed || !tcp.Connected) { errorMessage = $"Cannot reach {host}:{port}. TCP connection failed or timed out."; return false; }

            return true;
        }
        catch (SocketException sex) { errorMessage = $"Network error during TCP probe: {sex.Message}"; return false; }
        catch (Exception ex) { errorMessage = $"TCP probe failed: {ex.Message}"; return false; }
    }

    internal static string BuildConnectionStringForSource(ServerConnectionSettings mainSettings, string selectedHost, int selectedPort, out ServerConnectionSettings usedSettings)
    {
        if (string.IsNullOrEmpty(selectedHost))
        {
            ...
            if (connService is not SqlServerConnectionService)
                throw new InvalidOperationException("Expected SQL Server provider.");
    ...
    internal static bool TrySelectSourceInternal(ServerConnectionSettings mainSettings, out string connectionString, out ServerConnectionSettings usedSettings)
    {
        ...
        if (choice.StartsWith("Use link profile:"))
        {
            ...
            if (match == null) { TuiComponents.ShowError("Profile not found."); TuiComponents.Pause(); connectionString = string.Empty; usedSettings = mainSettings; return false; }
        ...
        else if (choice == "Ad-hoc IP (enter IP)")
        {
            ...
            if (string.IsNullOrEmpty(adh)) { connectionString = string.Empty; usedSettings = mainSettings; return false; }
            if (adh.Contains(","))
            {
            ...
            else { selectedHost = adh; selectedPort = 1433; }
        }
        else { selectedHost = string.Empty; }
